...
if (document.getElementById && document.childNodes && document.createElement) {
  if (!(window.MathJax && MathJax.Hub)) {
        ...
        return this.queue.Push(
          ...
          [
            function (d) {
              ...
              if (
                d.menuSettings.CHTMLpreview &&
                !MathJax.Extension["CHTML-preview"]
              ) {
    ...
    (function (n) {
      ...
      for (var p = b.length - 1; p >= 0; p--) {
        if ((b[p].src || "").match(e)) {
          ...
          v.root = b[p].src
            .replace(/(^|\/)[^\/]*(\?.*)?$/, "")
            .replace(
              /^(https?:)\/\/[0-9a-f]+(-[0-9a-f]+)?.ssl.cf1.rackcdn.com\//,
              "$1//cdn.mathjax.org/",
            );
      ...
      c.Browser = c.Insert(
        c.Insert(new String("Unknown"), { version: "0.0" }),
        a,
      );
      for (var u in a) {
      ...
      c.Browser.Select({
        ...
        Firefox: function (j) {
          if (
            (j.version === "0.0" || h.match(/Firefox/) == null) &&
