...
- `stylesheetsInline(getStylesheetContentOrOptions?: ((href: string) => Promise<string | null | undefined>) | { cache?: boolean; maxCacheSize?: number; cacheKeyPrefix?: string }, options?: { cache?: boolean; maxCacheSize?: number; cacheKeyPrefix?: string }): Promise<QtiTransformAPI>`: Fetch and inline stylesheet content into each `qti-stylesheet` tag that has an `href`. By default it caches CSS in `sessionStorage` (`cache: true`) and stores only files up to `50 * 1024` characters (`maxCacheSize`).
